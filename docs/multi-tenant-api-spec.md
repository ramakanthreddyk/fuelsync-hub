...
- **Employees** are tied to specific stations
...
interface User {
  ...
  station_id: number | null; // Only for employees
  is_active: boolean;
  created_at: string;
  updated_at: string;
}
...
```json
{
  ...
  "password": "password123",
  "role": "employee",
  "station_id": 1 // Required for employees
}
```
...
- Owner: Own stations only
- Employee: Assigned station only
...
- 1000 requests per hour for super admins
...
### Super Admin
- Can access all data across all stations
- Can manage all users and stations
- No data filtering applied

### Owner
- Can only access data for stations they own
- Can manage employees within their stations
- All queries automatically filtered by `owner_id`

### Employee
- Can only access data for their assigned station
- Cannot manage other users
- All queries automatically filtered by `station_id`

## Business Rules

1. **Owner Creation**: When creating an owner, optionally create their first station simultaneously
2. **Employee Assignment**: Employees must be assigned to a station and can only access that station's data
